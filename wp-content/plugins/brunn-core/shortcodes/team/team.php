<?php
namespace BrunnCore\CPT\Shortcodes\Team;
...
class Team implements lib\ShortcodeInterface {
	...
	public function vcMap() {
		...
		for ( $x = 1; $x < 6; $x ++ ) {
			$teamIconCollections = brunn_select_icon_collections()->getCollectionsWithSocialIcons();
			foreach ( $teamIconCollections as $collection_key => $collection ) {
				
				$team_social_icons_array[] = array(
					'type'       => 'dropdown',
					'heading'    => esc_html__( 'Social Icon ', 'brunn-core' ) . $x,
			...
			$team_social_icons_array[] = array(
				'type'       => 'textfield',
				'heading'    => esc_html__( 'Social Icon ', 'brunn-core' ) . $x . esc_html__( ' Link', 'brunn-core' ),
				'param_name' => 'team_social_icon_' . $x . '_link',
				'dependency' => array(
					'element' => 'team_social_icon_pack',
					'value'   => brunn_select_icon_collections()->getIconCollectionsKeys()
				)
			);
			
			$team_social_icons_array[] = array(
				'type'       => 'dropdown',
				'heading'    => esc_html__( 'Social Icon ', 'brunn-core' ) . $x . esc_html__( ' Target', 'brunn-core' ),
		...
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Team', 'brunn-core' ),
					'base'                      => $this->base,
					...
					'params'                    => array_merge(
						array(
							array(
								...
								'value'       => array(
									esc_html__( 'Info Below Image', 'brunn-core' )    => 'info-below-image',
									esc_html__( 'Info On Image Hover', 'brunn-core' ) => 'info-on-image'
								),
								'save_always' => true
							),
							array(
							...
							array(
								...
								'heading'    => esc_html__( 'Name', 'brunn-core' )
							),
							array(
								'type'        => 'dropdown',
								'param_name'  => 'team_name_tag',
								'heading'     => esc_html__( 'Name Tag', 'brunn-core' ),
							...
							array(
								...
								'heading'    => esc_html__( 'Name Color', 'brunn-core' ),
								'dependency' => array( 'element' => 'team_name', 'not_empty' => true )
							),
							array(
								'type'       => 'textfield',
								'param_name' => 'team_position',
								'heading'    => esc_html__( 'Position', 'brunn-core' )
							),
							array(
								'type'       => 'colorpicker',
								'param_name' => 'team_position_color',
								'heading'    => esc_html__( 'Position Color', 'brunn-core' ),
							...
							array(
								'type'       => 'colorpicker',
								'param_name' => 'team_text_holder_color',
								'heading'    => esc_html__( 'Text Holder Color', 'brunn-core' ),
								'dependency' => array( 'element' => 'type', 'value' => array( 'info-below-image' ) )
							),
							array(
								'type'        => 'dropdown',
								'param_name'  => 'team_social_icon_pack',
								'heading'     => esc_html__( 'Social Icon Pack', 'brunn-core' ),
								'value'       => array_merge( array( '' => '' ), brunn_select_icon_collections()->getIconCollectionsVCExclude( 'linea_icons' ) ),
								'save_always' => true
							),
						),
	...
	public function render( $atts, $content = null ) {
		...
		for ( $x = 1; $x <= $number_of_social_icons; $x ++ ) {
			...
			foreach ( brunn_select_icon_collections()->iconCollections as $collection_key => $collection ) {
		...
		$html = brunn_core_get_shortcode_module_template_part( 'templates/' . $params['type'], 'team', '', $params );
