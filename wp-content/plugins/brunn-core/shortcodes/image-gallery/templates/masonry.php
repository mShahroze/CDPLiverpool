		...
		<?php foreach ( $images as $image ) { ?>
			...
			if ( ! empty( $image_size_class ) ) {
				...
				if ( $image_size_class === 'large-width-height' ) {
					...
					$new_image_size = 'brunn_select_image_square';
				} else if ( $image_size_class === 'large-width' ) {
					$new_image_size = 'brunn_select_image_landscape';
				} else if ( $image_size_class === 'large-height' ) {
					$new_image_size = 'brunn_select_image_portrait';
				} else {
					$new_image_size = 'full';
				}
			}
				...
				<div class="qodef-ig-image-inner">
					...
					<?php } ?>
						<?php if ( is_array( $image_size ) && count( $image_size ) ) :
							echo brunn_select_generate_thumbnail( $image['image_id'], null, $image_size[0], $image_size[1] );
						else:
