<div class="qodef-reviews-list-info qodef-reviews-per-mark">
	<?php foreach ( $post_ratings as $rating ) { ?>
		<?php
		$average_rating     = brunn_core_post_average_rating( $rating );
		$rating_count       = $rating['count'];
		$rating_count_label = $rating_count === 1 ? esc_html__( 'Rating', 'brunn-core' ) : esc_html__( 'Ratings', 'brunn-core' );
				...
				<div class="qodef-reviews-number-wrapper">
					...
					<span class="qodef-stars-wrapper">
                        ...
                        <span class="qodef-reviews-count">
                            <?php esc_html_e( 'Rated', 'brunn-core' ) . ' ' . $average_rating . ' ' . esc_html__( 'out of', 'brunn-core' ) . ' ' . $rating_count . ' ' . $rating_count_label; ?>
                        </span>
                    </span>
				</div>
			</div>
			<div class="qodef-grid-col-8">
				<div class="qodef-rating-percentage-wrapper">
					...
					foreach ( $rating_marks as $item => $value ) {
						...
						echo do_shortcode( '[qodef_progress_bar percent="' . esc_attr( $percentage ) . '" title="' . esc_attr( $item ) . esc_attr__( ' stars', 'brunn-core' ) . '"]' );
					}
					?>
				</div>
			</div>
