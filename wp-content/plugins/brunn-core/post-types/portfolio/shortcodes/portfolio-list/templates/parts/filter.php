<?php if($filter == 'yes') {
	...
	?>
	<div class="qodef-pl-filter-holder" <?php brunn_select_inline_style($filter_holder_styles); ?>>
		<div class="qodef-plf-inner">
			<?php
			if(is_array($filter_categories) && count($filter_categories)){ ?>
				<ul <?php brunn_select_inline_style($filter_styles); ?>>
					<li class="qodef-pl-filter" data-filter="">
						<span><?php esc_html_e('Show all', 'brunn-core')?></span>
					</li>
