    ...
    <div class="qodef-tl-wrapper qodef-outer-space">
            ...
            <?php if ( is_array( $response->data ) && count( $response->data ) ) { ?>
                <ul class="qodef-twitter-list">
                    <?php foreach ( $response->data as $tweet ) { ?>
                        <?php
                            $params['tweet'] = $tweet;
                            echo brunn_twitter_get_shortcode_module_template_part('templates/item', 'twitter-list', '', $params);
                        ?>
                    <?php } ?>
                </ul>
            <?php } else { ?>
        ...
        <?php } else { ?>
            <div class="qodef-twitter-not-connected">
                <?php esc_html_e( 'It seams that you haven\'t connected with your Twitter account', 'brunn-twitter-feed' ); ?>
            </div>
        <?php } ?>
    </div>
</div>