...
namespace BrunnTwitter\Shortcodes\TwitterList;

use BrunnTwitter\Lib;
...
class TwitterList implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Twitter List', 'brunn-twitter-feed' ),
					'base'                      => $this->base,
					'category'                  => esc_html__( 'by BRUNN', 'brunn-twitter-feed' ),
					...
					'params'                    => array(
						array(
							'type'        => 'textfield',
							'param_name'  => 'user_id',
							'heading'     => esc_html__( 'User ID', 'brunn-twitter-feed' ),
							'admin_label' => true
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'number_of_columns',
							'heading'     => esc_html__( 'Number of Columns', 'brunn-twitter-feed' ),
							'value'       => array_flip( brunn_select_get_number_of_columns_array( false, array( 'one' ) ) ),
							'save_always' => true
						),
						array(
							'type'       => 'dropdown',
							'param_name' => 'space_between_columns',
							'heading'    => esc_html__( 'Space Between Columns', 'brunn-twitter-feed' ),
							'value'      => array_flip( brunn_select_get_space_between_items_array() )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'number_of_tweets',
							'heading'    => esc_html__( 'Number of Tweets', 'brunn-twitter-feed' )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'transient_time',
							'heading'    => esc_html__( 'Tweets Cache Time', 'brunn-twitter-feed' )
						)
					)
				)
			);
	...
	public function render( $atts, $content = null ) {
		...
		$twitter_api           = new \BrunnTwitterApi();
		...
		$html = brunn_twitter_get_shortcode_module_template_part( 'holder', 'twitter-list', '', $params );
