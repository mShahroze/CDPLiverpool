<div class="qodef-instagram-list-holder <?php echo esc_attr($holder_classes); ?>">
    <?php if ( is_array( $images_array ) && count( $images_array ) ) { ?>
	    <ul class="qodef-instagram-feed qodef-outer-space <?php echo esc_attr($carousel_classes); ?> clearfix" <?php echo brunn_select_get_inline_attrs( $data_attr ) ?>>
	    <?php
    ...
    <?php } else { ?>
        <div class="qodef-instagram-not-connected">
            <?php esc_html_e( 'It seams that you haven\'t connected with your Instagram account', 'brunn-instagram-feed' ); ?>
        </div>
    <?php } ?>
</div>