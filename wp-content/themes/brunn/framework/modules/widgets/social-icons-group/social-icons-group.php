...
if ( class_exists( 'BrunnSelectClassWidget' ) ) {
	class BrunnSelectClassClassIconsGroupWidget extends BrunnSelectClassWidget {
		public function __construct() {
			parent::__construct(
				'qodef_social_icons_group_widget',
				esc_html__( 'Brunn Social Icons Group Widget', 'brunn' ),
				array( 'description' => esc_html__( 'Use this widget to add a group of up to 6 social icons to a widget area.', 'brunn' ) )
			);
		...
		protected function setParams() {
			$this->params = array_merge(
				...
				brunn_select_icon_collections()->getSocialIconWidgetMultipleParamsArray( 6 ),
				array(
					array(
						'type'    => 'dropdown',
						'name'    => 'layout',
						'title'   => esc_html__( 'Icons Layout', 'brunn' ),
						'options' => array(
							''             => esc_html__( 'Default', 'brunn' ),
							'square-icons' => esc_html__( 'Square', 'brunn' ),
						)
					),
					array(
						'type'        => 'dropdown',
						'name'        => 'skin',
						'title'       => esc_html__( 'Square Icons Skin', 'brunn' ),
						'description' => esc_html__( 'This applies to the square layout', 'brunn' ),
						'options'     => array(
							''           => esc_html__( 'Dark Skin', 'brunn' ),
							'light-skin' => esc_html__( 'Light Skin', 'brunn' ),
						)
					),
					array(
						'type'    => 'dropdown',
						'name'    => 'alignment',
						'title'   => esc_html__( 'Text Alignment', 'brunn' ),
						'options' => array(
					...
					array(
						'type'  => 'textfield',
						'name'  => 'icon_size',
						'title' => esc_html__( 'Icons Size (px)', 'brunn' )
					),
					array(
						'type'  => 'colorpicker',
						'name'  => 'color',
		...
		public function widget( $args, $instance ) {
			...
			for ( $n = 1; $n <= 6; $n ++ ) {
				...
				<?php if ( ! empty( $icon_holder_html ) ) { ?>
                    <a class="qodef-social-icon-widget-holder qodef-icon-has-hover" <?php echo brunn_select_get_inline_attr( $hover_color, 'data-hover-color' ); ?> <?php brunn_select_inline_style( $icon_styles ) ?>
