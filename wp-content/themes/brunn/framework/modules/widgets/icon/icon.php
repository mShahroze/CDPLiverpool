...
if ( class_exists( 'BrunnSelectClassWidget' ) ) {
	class BrunnSelectClassIconWidget extends BrunnSelectClassWidget {
		public function __construct() {
			parent::__construct(
				'qodef_icon_widget',
				esc_html__( 'Brunn Icon Widget', 'brunn' ),
				array( 'description' => esc_html__( 'Add icons to widget areas', 'brunn' ) )
			);
		...
		protected function setParams() {
			$this->params = array_merge(
				brunn_select_icon_collections()->getIconWidgetParamsArray(),
				array(
					array(
						'type'  => 'textfield',
						'name'  => 'icon_text',
						'title' => esc_html__( 'Icon Text', 'brunn' )
					),
					array(
						'type'  => 'textfield',
						'name'  => 'link',
				...
				,
					array(
						...
						'title' => esc_html__( 'Space between Icon and Text (px)', 'brunn' )
					),
					array(
						'type'  => 'colorpicker',
						'name'  => 'icon_color',
					...
					array(
						'type'  => 'textfield',
						'name'  => 'text_size',
						'title' => esc_html__( 'Text Size (px)', 'brunn' )
					),
					array(
						'type'  => 'colorpicker',
						'name'  => 'text_color',
						'title' => esc_html__( 'Text Color', 'brunn' )
					),
					array(
						'type'        => 'textfield',
						'name'        => 'margin',
		...
		public function widget( $args, $instance ) {
			...
			$data   = array();
			$data[] = ! empty( $instance['icon_hover_color'] ) ? brunn_select_get_inline_attr( $instance['icon_hover_color'], 'data-hover-color' ) : '';
			$data   = implode( ' ', $data );
			...
			if ( ! empty( $instance['text_size'] ) ) {
				$text_styles[] = 'font-size: ' . brunn_select_filter_px( $instance['text_size'] ) . 'px';
			}
			if ( ! empty( $instance['text_color'] ) ) {
				$text_styles[] = 'color: ' . $instance['text_color'];
			}
			if ( ! empty( $instance['icon_text_padding'] ) ) {
				$text_styles[] = 'padding-left: ' . brunn_select_filter_px( $instance['icon_text_padding'] ) . 'px';;
			}
			...
			if ( ! empty( $instance['icon_pack'] ) ) {
				...
				if ( ! empty( $icon_class ) ) {
					...
					$icon_holder_html = '<span class="qodef-icon-element ' . esc_attr( $icon_class ) . '" ' . brunn_select_get_inline_style( $icon_styles ) . '></span>';
				}
			}
			...
			if ( ! empty( $instance['icon_text'] ) ) {
				$icon_text_html = '<span class="qodef-icon-text ' . esc_attr( $icon_text_class ) . '" ' . brunn_select_get_inline_style( $text_styles ) . '>' . esc_html( $instance['icon_text'] ) . '</span>';
			}
			?>

            <a <?php brunn_select_class_attribute( $holder_classes ); ?> <?php echo wp_kses_post( $data ); ?> href="<?php echo esc_url( $link ); ?>" target="<?php echo esc_attr( $target ); ?>" <?php echo brunn_select_get_inline_style( $holder_styles ); ?>>
