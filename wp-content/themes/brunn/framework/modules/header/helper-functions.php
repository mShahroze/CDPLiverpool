...
if ( ! function_exists( 'brunn_select_header_skin_class' ) ) {
	/**
	 * Function that adds header style class to body tag
	 */
	function brunn_select_header_skin_class( $classes ) {
		$header_style     = brunn_select_get_meta_field_intersect( 'header_style', brunn_select_get_page_id() );
		$header_style_404 = brunn_select_options()->getOptionValue( '404_header_style' );
	...
	add_filter( 'body_class', 'brunn_select_header_skin_class' );
}

if ( ! function_exists( 'brunn_select_sticky_header_behaviour_class' ) ) {
	/**
	 * Function that adds header behavior class to body tag
	 */
	function brunn_select_sticky_header_behaviour_class( $classes ) {
		$header_behavior = brunn_select_get_meta_field_intersect( 'header_behaviour', brunn_select_get_page_id() );
	...
	add_filter( 'body_class', 'brunn_select_sticky_header_behaviour_class' );
}

if ( ! function_exists( 'brunn_select_menu_dropdown_appearance' ) ) {
	/**
	 ...
	 */
	function brunn_select_menu_dropdown_appearance( $classes ) {
		$dropdown_menu_appearance = brunn_select_options()->getOptionValue( 'menu_dropdown_appearance' );
	...
	add_filter( 'body_class', 'brunn_select_menu_dropdown_appearance' );
}

if ( ! function_exists( 'brunn_select_header_class' ) ) {
	/**
	 ...
	 */
	function brunn_select_header_class( $classes ) {
		$id = brunn_select_get_page_id();
		
		$header_type = brunn_select_get_meta_field_intersect( 'header_type', $id );
		
		$classes[] = 'qodef-' . $header_type;
		
		$disable_menu_area_shadow = brunn_select_get_meta_field_intersect( 'menu_area_shadow', $id ) == 'no';
		if ( $disable_menu_area_shadow ) {
			$classes[] = 'qodef-menu-area-shadow-disable';
		}
		
		$disable_menu_area_grid_shadow = brunn_select_get_meta_field_intersect( 'menu_area_in_grid_shadow', $id ) == 'no';
		if ( $disable_menu_area_grid_shadow ) {
			$classes[] = 'qodef-menu-area-in-grid-shadow-disable';
		}
		
		$disable_menu_area_border = brunn_select_get_meta_field_intersect( 'menu_area_border', $id ) == 'no';
		if ( $disable_menu_area_border ) {
			$classes[] = 'qodef-menu-area-border-disable';
		}
		
		$disable_menu_area_grid_border = brunn_select_get_meta_field_intersect( 'menu_area_in_grid_border', $id ) == 'no';
		if ( $disable_menu_area_grid_border ) {
			$classes[] = 'qodef-menu-area-in-grid-border-disable';
		}
		
		if ( brunn_select_get_meta_field_intersect( 'menu_area_in_grid', $id ) == 'yes' &&
		     brunn_select_get_meta_field_intersect( 'menu_area_grid_background_color', $id ) !== '' &&
		     brunn_select_get_meta_field_intersect( 'menu_area_grid_background_transparency', $id ) !== '0'
		) {
			$classes[] = 'qodef-header-menu-area-in-grid-padding';
		}
		
		$disable_logo_area_border = brunn_select_get_meta_field_intersect( 'logo_area_border', $id ) == 'no';
		if ( $disable_logo_area_border ) {
			$classes[] = 'qodef-logo-area-border-disable';
		}
		
		$disable_logo_area_grid_border = brunn_select_get_meta_field_intersect( 'logo_area_in_grid_border', $id ) == 'no';
		if ( $disable_logo_area_grid_border ) {
			$classes[] = 'qodef-logo-area-in-grid-border-disable';
		}
		
		if ( brunn_select_get_meta_field_intersect( 'logo_area_in_grid', $id ) == 'yes' &&
		     brunn_select_get_meta_field_intersect( 'logo_area_grid_background_color', $id ) !== '' &&
		     brunn_select_get_meta_field_intersect( 'logo_area_grid_background_transparency', $id ) !== '0'
		) {
			$classes[] = 'qodef-header-logo-area-in-grid-padding';
		}
		
		$disable_shadow_vertical = brunn_select_get_meta_field_intersect( 'vertical_header_shadow', $id ) == 'no';
		if ( $disable_shadow_vertical ) {
			$classes[] = 'qodef-header-vertical-shadow-disable';
		}
		
		$disable_border_vertical = brunn_select_get_meta_field_intersect( 'vertical_header_border', $id ) == 'no';
	...
	add_filter( 'body_class', 'brunn_select_header_class' );
}

if ( ! function_exists( 'brunn_select_header_area_style' ) ) {
	/**
	 * Function that return styles for header area
	 */
	function brunn_select_header_area_style( $style ) {
		...
		$menu_area_background_color_rgba = brunn_select_rgba_color( $menu_area_background_color, $menu_area_background_transparency );
		...
		if ( $menu_area_height !== '' ) {
			$menu_area_style['height'] = brunn_select_filter_px( $menu_area_height ) . 'px !important';
		}
		...
		if ( $container_side_padding !== '' ) {
			...
			$current_style .= brunn_select_dynamic_css( $menu_container_selector, $menu_container_styles );
		}
		...
		$menu_area_grid_background_color_rgba = brunn_select_rgba_color( $menu_area_grid_background_color, $menu_area_grid_background_transparency );
		
		if ( ! empty( $menu_area_grid_background_color_rgba ) ) {
			$menu_area_grid_style['background-color'] = $menu_area_grid_background_color_rgba;
		}
		
		$current_style .= brunn_select_dynamic_css( $menu_area_selector, $menu_area_style );
		$current_style .= brunn_select_dynamic_css( $menu_area_grid_selector, $menu_area_grid_style );
		...
		if ( $dropdown_top_position !== '' ) {
			$dropdown_styles['top'] = brunn_select_filter_suffix( $dropdown_top_position, '%' ) . '%';
		}
		
		$dropdown_selector = array( $class_prefix . ' .qodef-page-header .qodef-drop-down .second' );
		
		$current_style .= brunn_select_dynamic_css( $dropdown_selector, $dropdown_styles );
		...
		$logo_area_background_color_rgba = brunn_select_rgba_color( $logo_area_background_color, $logo_area_background_transparency );
		...
		if ( $logo_area_height !== '' ) {
			$logo_area_style['height'] = brunn_select_filter_px( $logo_area_height ) . 'px !important';
		}
		...
		$logo_area_grid_background_color_rgba = brunn_select_rgba_color( $logo_area_grid_background_color, $logo_area_grid_background_transparency );
		...
		if ( ! empty( $logo_area_style ) ) {
			$current_style .= brunn_select_dynamic_css( $logo_area_selector, $logo_area_style );
		}
		
		if ( ! empty( $logo_area_grid_style ) ) {
			$current_style .= brunn_select_dynamic_css( $logo_area_grid_selector, $logo_area_grid_style );
		}
		
		$current_style = apply_filters( 'brunn_select_filter_add_header_page_custom_style', $current_style, $class_prefix, $page_id ) . $style;
		
		return $current_style;
	}
	
	add_filter( 'brunn_select_filter_add_page_custom_style', 'brunn_select_header_area_style' );
}