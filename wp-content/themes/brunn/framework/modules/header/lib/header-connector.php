...
namespace BrunnSelectNamespace\Modules\Header\Lib;

/**
 * Class BrunnSelectClassHeaderConnector
 *
...
class BrunnSelectClassHeaderConnector {
	...
	public function connect( $config = array() ) {
		do_action( 'brunn_select_action_pre_header_connect' );
		...
		if ( ! empty( $config['affect_content'] ) ) {
			add_filter( 'brunn_select_filter_content_elem_style_attr', array( $this, 'contentMarginFilter' ) );
		}
		
		if ( ! empty( $config['affect_title'] ) ) {
			add_filter( 'brunn_select_filter_title_content_padding', array( $this, 'titlePaddingFilter' ) );
		}
		
		do_action( 'brunn_select_action_after_header_connect' );
	}
	 ...
	 *
	 * Hooks to brunn_select_filter_title_content_padding filter
	 *
	 * @return int
	 */
