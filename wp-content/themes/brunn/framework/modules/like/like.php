...
class BrunnSelectClassLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_brunn_select_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_brunn_select_like', array( $this, 'ajax' ) );
	}
	...
	public function like_post( $post_id, $action = 'get', $type = '' ) {
		...
		switch ( $action ) {
			...
			case 'get':
				...
				$return_value = $icon . "<span>" ." ". esc_html__( 'Like', 'brunn' ) . "</span>";
			...
			case 'update':
				...
				if ( $like_count == 1){
					$like_text = esc_html__( 'Like', 'brunn' );
				} else{
					$like_text = esc_html__( 'Likes', 'brunn' );
				}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'brunn' );
		
		if ( isset( $_COOKIE[ 'qodef-like_' . $post->ID ] ) ) {
			$class = 'qodef-like liked';
			$title = esc_html__( 'You already like this!', 'brunn' );
		}
...
if ( ! function_exists( 'brunn_select_create_like' ) ) {
	function brunn_select_create_like() {
		BrunnSelectClassLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'brunn_select_create_like' );
}