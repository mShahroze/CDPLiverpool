...
class SelectSkin extends BrunnSelectClassSkinAbstract {
	
	/**
	 * Skin constructor. Hooks to brunn_select_admin_scripts_init and brunn_select_enqueue_admin_styles
	 */
	public function __construct() {
		$this->skinName = 'select';
		
		// Admin register scripts and styles
		add_action( 'brunn_select_action_admin_scripts_init', array( $this, 'registerStyles' ) );
		add_action( 'brunn_select_action_admin_scripts_init', array( $this, 'registerScripts' ) );
		
		// Admin enqueue scripts and styles
		add_action( 'brunn_select_action_enqueue_admin_styles', array( $this, 'enqueueStyles' ) );
		add_action( 'brunn_select_action_enqueue_admin_scripts', array( $this, 'enqueueScripts' ) );
		
		// Admin enqueue meta box scripts and styles
		add_action( 'brunn_select_action_enqueue_meta_box_styles', array( $this, 'enqueueStyles' ) );
		add_action( 'brunn_select_action_enqueue_meta_box_scripts', array( $this, 'enqueueScripts' ) );
		
		// Change options map order for this skin
		add_filter( 'brunn_select_filter_options_map_position', array( $this, 'setOptionsMapPosition' ), 10, 2 );
	...
	public function registerScripts() {
		...
		foreach ( $this->scripts as $scriptHandle => $script ) {
			brunn_select_register_skin_script( $scriptHandle, $script['path'], $script['dependency'] );
		}
	}
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
            brunn_select_register_skin_style($styleHandle, $stylePath);
        }
    }
    /**
	...
	public function renderOptions() {
        ...
        $tab    = brunn_select_get_admin_tab();
        $active_page = $brunn_select_global_Framework->qodeOptions->getAdminPageFromSlug($tab);
     ...
     *
     * @see BrunnSelectClassSkinAbstract::loadTemplatePart()
     */
     ...
     *
	 * @param BrunnSelectClassAdminPage $page current page to load
     *
     * @see BrunnSelectClassSkinAbstract::loadTemplatePart()
     */
