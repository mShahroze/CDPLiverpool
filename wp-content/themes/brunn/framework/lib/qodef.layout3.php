...
class BrunnSelectClassInstagramFramework implements iBrunnSelectInterfaceRender {
	public function render( $factory ) {
		...
		if ( ! get_option( 'brunn_instagram_code' ) ) {
			...
			if ( ! empty( $_GET['code'] ) ) {
				...
				$message = esc_html__( 'You have successfully connected with your Instagram account. If you have any issues fetching data from Instagram try reconnecting.', 'brunn' );
		...
		$buttonText = $instagram_api->hasUserConnected() ? esc_html__( 'Re-connect with Instagram', 'brunn' ) : esc_html__( 'Connect with Instagram', 'brunn' );
		?>
		<?php if ( $message !== '' ) { ?>
			<div class="alert alert-success">
				<span><?php echo esc_html( $message ); ?></span>
