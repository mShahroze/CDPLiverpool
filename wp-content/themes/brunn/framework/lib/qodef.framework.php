...
/*
   Class: BrunnSelectClassFramework
...
class BrunnSelectClassFramework {
	...
	private function __construct() {
		$this->qodeOptions         = BrunnSelectClassOptions::get_instance();
		$this->qodeMetaBoxes       = BrunnSelectClassMetaBoxes::get_instance();
		$this->qodeTaxonomyOptions = BrunnSelectClassTaxonomyOptions::get_instance();
		$this->qodeUserOptions     = BrunnSelectClassUserOptions::get_instance();
		$this->qodeDashboardOptions = BrunnSelectClassDashboardOptions::get_instance();
	}
	...
	public function setSkin( BrunnSelectClassSkinAbstract $skinObject ) {
		$this->skin = $skinObject;
	}
}

/**
 * Class BrunnSelectClassSkinManager
 *
...
class BrunnSelectClassSkinManager {
	...
	/**
	 * @see BrunnSelectClassSkinManager::setSkin()
	 */
	...
	private function setSkin( $skinName = SELECT_PROFILE_SLUG ) {
		if ( $skinName !== '' ) {
			if ( file_exists( get_template_directory() . '/framework/admin/skins/' . $skinName . '/skin.php' ) ) {
				...
				$skinName = ucfirst( $skinName ) . esc_html__( 'Skin', 'brunn' );
...
/**
 * Class BrunnSelectClassSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class BrunnSelectClassSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	public function loadTemplatePart( $template, $params = array() ) {
		...
		if ( $template !== '' ) {
			include( brunn_select_find_template_path( 'framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php' ) );
		}
	}
	...
	public function setShortcodeJSParams() { ?>
		<script>
			window.qodeSCIcon = '<?php echo brunn_select_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
			window.qodeSCLabel = '<?php echo esc_html( ucfirst( $this->skinName ) ); ?> <?php esc_html_e( 'Shortcodes', 'brunn' ) ?>';
		</script>
	<?php }
...
class BrunnSelectClassOptions {
	...
	public function getOptionValue( $key ) {
		...
		if ( array_key_exists( $key, $brunn_select_global_options ) ) {
			return $brunn_select_global_options[ $key ];
...
/*
   Class: BrunnSelectClassAdminPage
   A class that initializes Select Admin Page
*/
class BrunnSelectClassAdminPage implements iBrunnSelectInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $slug;
	public $title;
	...
	function __construct( $slug = "", $title_label = "", $icon = "" ) {
		$this->layout  = array();
		$this->factory = new BrunnSelectClassFieldFactory();
		$this->slug    = $slug;
	...
	public function renderChild( iBrunnSelectInterfaceRender $child ) {
		$child->render( $this->factory );
	}
}

/*
   Class: BrunnSelectClassMetaBoxes
...
/*
   Class: BrunnSelectClassMetaBox
   A class that initializes Select Meta Box
*/
class BrunnSelectClassMetaBox implements iBrunnSelectInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	public $title;
	...
	public function setTitle( $label ) {
		...
		return $brunn_select_global_Framework->getSkin()->getSkinLabel() . ' ' . $label;
	}
}

/*
   Class: BrunnSelectClassTaxonomyOptions
   A class that initializes BrunnSelectClass Taxonomy Options
*/
class BrunnSelectClassTaxonomyOptions {
	private static $instance;
...
/*
   Class: BrunnSelectClassTaxonomyOption
   A class that initializes BrunnSelectClass Taxonomy Option
*/
class BrunnSelectClassTaxonomyOption implements iBrunnSelectInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new BrunnSelectClassTaxonomyFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: BrunnSelectClassUserOptions
   A class that initializes BrunnSelectClass User Options
*/
class BrunnSelectClassUserOptions {
	private static $instance;
	public $userOptions;
...
/*
   Class: BrunnSelectClassUserOption
   A class that initializes BrunnSelectClass User Option
*/
class BrunnSelectClassUserOption implements iBrunnSelectInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new BrunnSelectClassUserFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: BrunnSelectClassDashboardOptions
   A class that initializes BrunnSelectClass Dashboard Options
*/
class BrunnSelectClassDashboardOptions {
	private static $instance;
...
/*
   Class: BrunnSelectClassDashboardOption
   A class that initializes BrunnSelectClass Dashboard Option
*/
class BrunnSelectClassDashboardOption implements iBrunnSelectInterfaceLayoutNode {
	public $layout;
	private $factory;
	
	function __construct() {
		$this->layout        = array();
		$this->factory       = new BrunnSelectClassDashboardFieldFactory();
	}
...
if ( ! function_exists( 'brunn_select_init_framework_variable' ) ) {
	function brunn_select_init_framework_variable() {
		global $brunn_select_global_Framework;
		
		$brunn_select_global_Framework = BrunnSelectClassFramework::get_instance();
		$qodeSkinManager                 = new BrunnSelectClassSkinManager();
		$brunn_select_global_Framework->setSkin( $qodeSkinManager->getSkin() );
	}
	
	add_action( 'brunn_select_action_before_options_map', 'brunn_select_init_framework_variable' );
}
?>